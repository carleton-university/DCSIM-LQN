...
public class ManagementMetrics extends MetricCollection {
  ...
  Map<Class<? extends MessageEvent>, Long>
      messageCount =
      new HashMap<Class<? extends MessageEvent>, Long>();
  Map<Class<? extends MessageEvent>, Double>
      messageBw =
      new HashMap<Class<? extends MessageEvent>, Double>();
  Map<Class<?>, Long> migrationCount = new HashMap<Class<?>, Long>();
  long placements;

  public enum MigrationType {
    INTRARACK, INTRACLUSTER, INTERCLUSTER;
  }
  ...
  public List<Pair<String, Object>> getMetricValues() {
    List<Pair<String, Object>> metrics = new ArrayList<>();

    for (Entry<Class<? extends MessageEvent>, Long> entry : getMessageCount().entrySet()) {
      metrics
          .add(new ImmutablePair<String, Object>("messages-" + entry.getKey().getName(),
                                                 entry.getValue()));
    }
    for (Entry<Class<? extends MessageEvent>, Double> entry : getMessageBw().entrySet()) {
      metrics
          .add(new ImmutablePair<String, Object>("messageBW-" + entry.getKey().getName(),
                                                 entry.getValue()));
    }
    for (Entry<Class<?>, Long> entry : getMigrationCount().entrySet()) {
      metrics.add(
          new ImmutablePair<String, Object>("migrations-" + entry.getKey().getName(),
                                            entry.getValue()));
    }
    metrics.add(new ImmutablePair<String, Object>("migrations-intrarack", intrarack));
    metrics.add(new ImmutablePair<String, Object>("migrations-intracluster", intracluster));
    metrics.add(new ImmutablePair<String, Object>("migrations-intercluster", intercluster));
