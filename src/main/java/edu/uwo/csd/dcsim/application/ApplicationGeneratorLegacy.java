...
import edu.uwo.csd.dcsim.application.events.ShutdownApplicationEvent;
import edu.uwo.csd.dcsim.application.events.SpawnApplicationEvent;
...
/**
 * The ApplicationGeneratorLegacy class generates new Applications and submits them to a data centre
 * based on given parameters, using VmPlacementEvent messages containing a list of VM requests
 * rather than using ApplicationPlacementEvent messages. This is presently required by 'legacy'
 * placement policies that do not accept ApplicationPlacementEvent.
 *
...
public abstract class ApplicationGeneratorLegacy implements SimulationEventListener {
  ...
  List<Pair<Long, Double>> applicationsPerHour;
  int currentRate = -1;
  long startTime = 0;

  protected Simulation simulation;

  public ApplicationGeneratorLegacy(Simulation simulation, AutonomicManager dcTarget,
                                    List<Pair<Long, Double>> applicationsPerHour) {
    this(simulation, dcTarget, null, applicationsPerHour);
  }

  public ApplicationGeneratorLegacy(Simulation simulation, AutonomicManager dcTarget,
                                    double applicationsPerHour) {
    this(simulation, dcTarget, null, applicationsPerHour);
  }
  ...
  public ApplicationGeneratorLegacy(Simulation simulation, AutonomicManager dcTarget,
                                    RealDistribution lifespanDist, double applicationsPerHour) {
  ...
  private void setArrivalRate(double applicationsPerHour) {
    ...
    simulation.getLogger()
        .debug("Service Arrival Rate set to " + applicationsPerHour + " services-per-hour");
  }
  ...
  private void sendNextRateChangeEvent() {
    ...
    if (currentRate == applicationsPerHour.size() - 1) {
      startTime = startTime + applicationsPerHour.get(currentRate).getLeft();
    }

    long
        nextEvent =
        (applicationsPerHour.get((currentRate + 1) % applicationsPerHour.size()).getLeft())
        + startTime;
  ...
  private void spawnApplication() {
    ...
    List<VmAllocationRequest> vmAllocationRequests = application.createInitialVmRequests();
  ...
  private void shutdownApplication(Application application) {
    ...
    if (application.canShutdown()) {
      ...
      simulation.sendEvent(new ShutdownApplicationEvent(this, application),
  ...
  public final void handleEvent(Event e) {
    if (e instanceof SpawnApplicationEvent) {
      ...
      if (previousRate == -1 || !applicationsPerHour.get(currentRate).getRight()
          .equals(applicationsPerHour.get(previousRate).getRight())) {
        setArrivalRate(applicationsPerHour.get(currentRate).getRight());
    ...
    public void eventCallback(Event e) {
      ...
      if (placementEvent.getFailedRequests().isEmpty()) {
        ...
        if (lifespanDist != null) {
          ...
          simulation
              .sendEvent(new ShutdownApplicationEvent(ApplicationGeneratorLegacy.this, application),
                         simulation.getSimulationTime() + lifeSpan);
        }
      } else {
        simulation.getLogger().debug("Application Placement Failed");
        simulation.getSimulationMetrics().getApplicationMetrics()
            .incrementApplicationPlacementsFailed();
      }
    }
