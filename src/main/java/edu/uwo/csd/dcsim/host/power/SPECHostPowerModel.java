...
/**
 * SPECHostPowerModel is a HostPowerModel that uses power utilization data in the form provided by
 * the SPECpower benchmark to determine host power consumption.
 *
...
public class SPECHostPowerModel implements HostPowerModel {
  ...
  @Override
  public double getPowerConsumption(Host.HostState state, double cpuUtil) {
    ...
    switch (state) {
      ...
      case POWERING_ON:
      case SUSPENDING:
        return getPowerConsumption(cpuUtil);
