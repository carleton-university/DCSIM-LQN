...
public class MigrationAction extends ManagementAction {
  ...
  private boolean
      verifyVm =
      false;
  //when true, the action will first verify that the migrating VM hasn't left the DC prior to the action executing
   ...
   *
   * @param triggeringEntity The SimulationEntity (VMRelocationPolicy, VMConsolidiationPolicy, etc.)
   *                         that is triggering this migration
   */
  public void execute(Simulation simulation, Object triggeringEntity) {
    ...
    if (verifyVm) {
      if (source.getVMAllocation(vmId) == null) {
        simulation.getLogger().debug(
            triggeringEntity.getClass().getName() + " Migrating VM #" + vmId + " from Host #"
            + source.id() + " is not longer present on the source host");
        completeAction();
        return;
      }
    }
    ...
    if (simulation.isRecordingMetrics()) {
      ...
      Rack sourceRack = simulation.host2Rack(source);
      Rack targetRack = simulation.host2Rack(target);
      if (sourceRack == targetRack) {
        simulation.getSimulationMetrics().getManagementMetrics()
            .addMigration(triggeringEntity.getClass(), MigrationType.INTRARACK);
      } else if (simulation.rack2Cluster(sourceRack) == simulation.rack2Cluster(targetRack)) {
        simulation.getSimulationMetrics().getManagementMetrics()
            .addMigration(triggeringEntity.getClass(), MigrationType.INTRACLUSTER);
      } else {
        simulation.getSimulationMetrics().getManagementMetrics()
            .addMigration(triggeringEntity.getClass(), MigrationType.INTERCLUSTER);
      }
    }

    //TODO improve logging output
    simulation.getLogger().debug(
        triggeringEntity.getClass().getName() + " Migrating VM #" + vmId + " from Host #" + source
            .id() + " to #" + target.id());
  }
