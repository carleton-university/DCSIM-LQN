...
public class InstantiateVmAction extends ManagementAction {
  ...
  public InstantiateVmAction(HostData target, VmAllocationRequest vmAllocationRequest,
  ...
  public void execute(Simulation simulation, Object triggeringEntity) {
    ...
    if (target.getCurrentStatus().getState() != Host.HostState.ON
        && target.getCurrentStatus().getState() != Host.HostState.POWERING_ON) {
      simulation
          .sendEvent(new PowerStateEvent(target.getHost(), PowerStateEvent.PowerState.POWER_ON));
    }

    InstantiateVmEvent
        instantiateEvent =
        new InstantiateVmEvent(target.getHostManager(), vmAllocationRequest);
    if (placementEvent != null) {
      placementEvent.addEventInSequence(instantiateEvent);
    }
