...
public class AutonomicManager implements SimulationEventListener {
  ...
  private final Map<Class<? extends ManagerCapability>, Object> capabilities = new HashMap<>();
  private final Map<RepeatingPolicyExecutionEvent, Policy> policyExecutionEvents = new HashMap<>();
  private final Map<Policy, RepeatingPolicyExecutionEvent> policyToExectionEvent = new HashMap<>();

  private Host container = null;
  //if this AutonomicManager is running within a Host, it is stored here
  ...
  private void addCapability(ManagerCapability capability) {
    ...
    if (!policies.contains(policy)) {
      ...
      if (policy.checkCapabilities(this)) {
        ...
        throw new RuntimeException(
            "Policy capability check failed for " + policy.getClass().toString());
      }
    }
  }
  ...
  public void installPolicy(Policy policy, long executionInterval, long startTime) {
    ...
    RepeatingPolicyExecutionEvent event =
        new RepeatingPolicyExecutionEvent(simulation, this, executionInterval);
  ...
  public void handleEvent(Event e) {
    ...
    if (container != null && !(container.getState() == Host.HostState.ON
                               || container.getState() == Host.HostState.POWERING_ON)) {
